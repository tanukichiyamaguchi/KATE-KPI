...
// ============================================================================
// Fixture P — 今月の着地見込み（store.forecast / staff.forecast）
// ============================================================================
// asOf=2026-07-10。7月: 会計済み 10,000+20,000=30,000、受付待ち(7/20) 8,000。
// confirmed=38,000。pace=round(30,000/10日*31日)=93,000（経過10日≥7で算出）。
h('■ Fixture P: 着地見込み（確定ベース＋暦日ペース）');
{
  const rows = [
    rec({ staff: 'A', custKey: 'A0', date: '2026-06-10', kaikeiTotal: 50000 }),
    rec({ staff: 'A', custKey: 'A1', date: '2026-07-01', kaikeiTotal: 10000 }),
    rec({ staff: 'A', custKey: 'A2', date: '2026-07-05', kaikeiTotal: 20000 }),
    rec({ staff: 'A', custKey: 'A3', status: '受付待ち', date: '2026-07-20', yoyakuTotal: 8000 })
  ];
  const R = engine.compute(rows, { asOf: '2026-07-10' });
  const f = R.store.forecast;
  check('store actual', f.actual, 30000);
  check('store expected（受付待ち見込み）', f.expected, 8000);
  check('store confirmed（確定ベース着地）', f.confirmed, 38000);
  check('daysElapsed', f.daysElapsed, 10);
  check('daysInMonth', f.daysInMonth, 31);
  check('pace = round(30000/10*31)', f.pace, 93000);
  const sf = R.staff.filter(s => s.name === 'A')[0].forecast;
  check('staff confirmed はストアと同値', sf.confirmed, 38000);
  check('staff pace はストアと同値', sf.pace, 93000);
}

// ============================================================================
// Fixture P2 — 着地見込みのエッジ: 月初7日未満は pace=null、当月データ無しはゼロ
// ============================================================================
h('■ Fixture P2: 着地見込みのエッジケース');
{
  // 会計明細のみ（受付待ちなし）× asOf 3日目 → expected=0、pace はまだ測れない
  const rows1 = [
    rec({ staff: 'A', custKey: 'B0', date: '2026-06-10', kaikeiTotal: 50000 }),
    rec({ staff: 'A', custKey: 'B1', date: '2026-07-01', kaikeiTotal: 10000 })
  ];
  const R1 = engine.compute(rows1, { asOf: '2026-07-03' });
  check('completedOnly: expected=0', R1.store.forecast.expected, 0);
  check('completedOnly: confirmed=actual', R1.store.forecast.confirmed, 10000);
  check('経過3日: pace は null', R1.store.forecast.pace === null ? 1 : 0, 1, 0);
  // 当月にデータが1件も無い → confirmed 0（正当な0）、スタッフ版は null
  const rows2 = [rec({ staff: 'A', custKey: 'C0', date: '2026-06-10', kaikeiTotal: 50000 })];
  const R2 = engine.compute(rows2, { asOf: '2026-07-10' });
  check('当月データ無し: store actual=0', R2.store.forecast.actual, 0);
  check('当月データ無し: store confirmed=0', R2.store.forecast.confirmed, 0);
  check('当月データ無し: store pace は null', R2.store.forecast.pace === null ? 1 : 0, 1, 0);
  check('当月データ無し: staff forecast は null', R2.staff[0].forecast === null ? 1 : 0, 1, 0);
}

// ============================================================================
// Fixture Q — 次回予約ストリーク（nextStreak）
// ============================================================================
// 「次の予約を確保できた来店(T)」は、その顧客に後日の予約（受付待ち 8/10）を
// 持たせて作る。asOf=2026-08-01、全来店が7日以上前なので保留スキップは発生しない。
h('■ Fixture Q: 次回予約ストリーク');
{
  // パターン [T,T,F,T,T] → best=2, current=2
  function visit(ck, date) { return rec({ staff: 'X', custKey: ck, date: date, kaikeiTotal: 5000 }); }
  function nextRes(ck) { return rec({ staff: 'X', custKey: ck, status: '受付待ち', date: '2026-08-10', yoyakuTotal: 5000 }); }
  const rows1 = [
    visit('Q1', '2026-07-01'), nextRes('Q1'),
    visit('Q2', '2026-07-05'), nextRes('Q2'),
    visit('Q3', '2026-07-10'),                 // F
    visit('Q4', '2026-07-15'), nextRes('Q4'),
    visit('Q5', '2026-07-20'), nextRes('Q5')
  ];
  const R1 = engine.compute(rows1, { asOf: '2026-08-01' });
  const st1 = R1.staff.filter(s => s.name === 'X')[0].nextStreak;
  check('[T,T,F,T,T] best', st1.best, 2);
  check('[T,T,F,T,T] current', st1.current, 2);
  check('[T,T,F,T,T] n（対象来店数）', st1.n, 5);

  // パターン [T,T,T,F,T] ＋ 直近3日前の未再予約来店（保留＝除外） → best=3, current=1
  const rows2 = [
    visit('Q1', '2026-07-01'), nextRes('Q1'),
    visit('Q2', '2026-07-05'), nextRes('Q2'),
    visit('Q3', '2026-07-10'), nextRes('Q3'),
    visit('Q4', '2026-07-15'),                 // F（17日前 → 対象）
    visit('Q5', '2026-07-20'), nextRes('Q5'),
    visit('Q6', '2026-07-29')                  // 未再予約・3日前 → 保留（対象外）
  ];
  const R2 = engine.compute(rows2, { asOf: '2026-08-01' });
  const st2 = R2.staff.filter(s => s.name === 'X')[0].nextStreak;
  check('[T,T,T,F,T]+保留 best', st2.best, 3);
  check('[T,T,T,F,T]+保留 current（保留で0に戻らない）', st2.current, 1);
  check('[T,T,T,F,T]+保留 n（保留は数えない）', st2.n, 5);

  // 会計明細のみ（受付待ちなし）: 直近30日の来店は一律除外。
  // C: 5/1(T: 6/1に再来店)、6/1(T: 7/25に再来店)、7/25(7日前 → 除外)
  const rows3 = [
    rec({ staff: 'X', custKey: 'C', name: 'C', date: '2026-05-01', kaikeiTotal: 5000 }),
    rec({ staff: 'X', custKey: 'C', name: 'C', date: '2026-06-01', kaikeiTotal: 5000 }),
    rec({ staff: 'X', custKey: 'C', name: 'C', date: '2026-07-25', kaikeiTotal: 5000 })
  ];
  const R3 = engine.compute(rows3, { asOf: '2026-08-01' });
  const st3 = R3.staff.filter(s => s.name === 'X')[0].nextStreak;
  check('completedOnly: 30日未満は除外（n=2）', st3.n, 2);
  check('completedOnly: current=2', st3.current, 2);
  check('completedOnly: best=2', st3.best, 2);

  // 来店ゼロのスタッフ（キャンセルのみ） → null
  const rows4 = rows1.concat([rec({ staff: 'Z', custKey: 'Z1', status: 'お客様キャンセル', date: '2026-07-10' })]);
  const R4 = engine.compute(rows4, { asOf: '2026-08-01' });
  const stZ = R4.staff.filter(s => s.name === 'Z')[0];
  check('来店ゼロ: nextStreak は null', stZ.nextStreak === null ? 1 : 0, 1, 0);
}

// ============================================================================
// Fixture R — 新規客の育成（nurturing / secondReached / 指名率）
// ============================================================================
// asOf=2026-07-15。C1: 来店1回+7/20の受付待ち → 育成中。C2: 来店1回のみ。
// C3: 6/10と7/5の2回 → 今月2回目到達。C4: 5/10と6/15 → 2回目は先月。
// 指名: 6月の担当来店4件（C1,C2,C3,C4）中、C2のみ指名予約 → 6月 1/4=0.25。
h('■ Fixture R: 新規客の育成と指名率');
{
  const rows = [
    rec({ staff: 'S', custKey: 'C1', name: 'C1', date: '2026-06-01', kaikeiTotal: 5000, shimei: '指名なし' }),
    rec({ staff: 'S', custKey: 'C1', name: 'C1', status: '受付待ち', date: '2026-07-20', yoyakuTotal: 5000 }),
    rec({ staff: 'S', custKey: 'C2', name: 'C2', date: '2026-06-05', kaikeiTotal: 5000, shimei: '指名予約' }),
    rec({ staff: 'S', custKey: 'C3', name: 'C3', date: '2026-06-10', kaikeiTotal: 5000, shimei: '指名なし' }),
    rec({ staff: 'S', custKey: 'C3', name: 'C3', date: '2026-07-05', kaikeiTotal: 5000, shimei: '指名なし' }),
    rec({ staff: 'S', custKey: 'C4', name: 'C4', date: '2026-05-10', kaikeiTotal: 5000, shimei: '指名なし' }),
    rec({ staff: 'S', custKey: 'C4', name: 'C4', date: '2026-06-15', kaikeiTotal: 5000, shimei: '指名なし' })
  ];
  const R = engine.compute(rows, { asOf: '2026-07-15' });
  const s = R.staff.filter(x => x.name === 'S')[0];
  check('育成中（2回目の予約が入っている新規客）', s.nurturing, 1);
  check('今月2回目のご来店', s.secondReached, 1);
  check('meta.anyShimei', R.meta.anyShimei ? 1 : 0, 1, 0);
  const jun = s.monthly.filter(m => m.m === '2026-06')[0];
  check('6月 shimeiCnt', jun.shimeiCnt, 1);
  check('6月 shimeiRate (1/4)', jun.shimeiRate, 0.25, 1e-9);

  // 指名列が全行空 → anyShimei false・shimeiRate null
  const rows2 = [rec({ staff: 'S', custKey: 'D1', date: '2026-06-01', kaikeiTotal: 5000 })];
  const R2 = engine.compute(rows2, { asOf: '2026-07-15' });
  check('指名データ無し: anyShimei は false', R2.meta.anyShimei ? 1 : 0, 0, 0);
  const jun2 = R2.staff[0].monthly.filter(m => m.m === '2026-06')[0];
  check('指名データ無し: shimeiRate は null', jun2.shimeiRate === null ? 1 : 0, 1, 0);
  // 会計明細のみ（受付待ちなし） → nurturing は 0 ではなく null
  check('completedOnly: nurturing は null', R2.staff[0].nurturing === null ? 1 : 0, 1, 0);
  check('completedOnly: secondReached は数値の0', R2.staff[0].secondReached, 0);
}

// ============================================================================
// Fixture S — 時間あたり売上（hourlyRev / hourlyRevRecent はプール平均）
// ============================================================================
// 5月: 60,000円/600分 → 6,000円/h。6月: 30,000円/200分 → 9,000円/h。
// 直近3ヶ月プール = 90,000 ÷ (800分/60) = 6,750。
// 月次単純平均なら (6,000+9,000)/2=7,500 になるため、プール規約の回帰を検出できる。
h('■ Fixture S: 時間あたり売上（プール平均）');
{
  const rows = [
    rec({ staff: 'H', custKey: 'H1', date: '2026-05-10', kaikeiTotal: 30000, dur: 300 }),
    rec({ staff: 'H', custKey: 'H2', date: '2026-05-20', kaikeiTotal: 30000, dur: 300 }),
    rec({ staff: 'H', custKey: 'H3', date: '2026-06-10', kaikeiTotal: 30000, dur: 200 })
  ];
  const R = engine.compute(rows, { asOf: '2026-08-01' });
  const s = R.staff.filter(x => x.name === 'H')[0];
  const may = s.hourlyRev.filter(h => h.m === '2026-05')[0];
  const jun = s.hourlyRev.filter(h => h.m === '2026-06')[0];
  check('5月 ¥/h', may.value, 6000);
  check('6月 ¥/h', jun.value, 9000);
  check('直近3ヶ月プール平均（6750≠月次平均7500）', s.hourlyRevRecent, 6750);

  // 所要時間の無いデータ → 両方 null
  const rows2 = [rec({ staff: 'H', custKey: 'H1', date: '2026-05-10', kaikeiTotal: 30000 })];
  const R2 = engine.compute(rows2, { asOf: '2026-08-01' });
  check('dur無し: hourlyRev は null', R2.staff[0].hourlyRev === null ? 1 : 0, 1, 0);
  check('dur無し: hourlyRevRecent は null', R2.staff[0].hourlyRevRecent === null ? 1 : 0, 1, 0);
}
