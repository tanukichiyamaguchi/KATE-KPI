# Atelier Éclat — KATE サロン ダッシュボード
...
- **入れるだけ** — 予約データ（`.xlsx` / `.csv`）をドラッグ&ドロップすると全指標を自動再計算。
...
- **一流のデザイン** — シャンパン地に深いプラム、ジュエルトーンのデータ、ローズゴールドの差し色（“Atelier Éclat”）。
  Fraunces のエディトリアル数字、光沢メーター、花びらファネル、なめらかなアニメーション。
...
リポジトリの **Settings → Pages** で、対象ブランチの `/ (root)` を公開に設定するだけ。
（`.nojekyll` を同梱済みなので `assets/` はそのまま配信されます。）

### 3. 自分の予約データで見る
`データ` タブを開き、「予約データ」シートを含む `.xlsx` か、その内容を書き出した `.csv` を
ドロップ／選択します。列は日本語の見出し名で自動認識するため、列の並び順は自由です。
...
デザインは複数のデザイナーエージェントによる探索と統合を経た「Atelier Éclat」ブランドに基づきます
（`docs/DESIGN-BRIEF.md`）。データ色はカラーバリアフリー検証（`dataviz` 基準）に合格した
ジュエルトーンのパレットを使用し、ローズゴールドは装飾（クローム）専用でデータ値には用いません。
