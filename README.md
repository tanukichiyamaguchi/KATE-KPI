...
- **一流のデザイン** — シャンパン地に深いプラム、ジュエルトーンのデータ、ローズゴールドの差し色（“Atelier Éclat”）。
  Noto Sans JP の読みやすい数字、光沢メーター、花びらファネル、なめらかなアニメーション。
...
**一度だけ** リポジトリの **Settings → Pages → Source を「GitHub Actions」** に設定してください。
（ブランチ配信を使う場合は、対象ブランチの `/ (root)` を選ぶだけでもOK。`.nojekyll` 同梱。）
...
assets/
  css/styles.css           デザインシステム（Atelier Éclat）※Noto Sans JP
  js/
